...
import { TABS, CATEGORY_COLORS, drinkWindowStatus } from '../components/cellar/constants'
...
function DrinkWindowAlerts({ bottles }) {
  const year = new Date().getFullYear()
  const overdue = bottles.filter(b => {
    const s = drinkWindowStatus(b)
  ...
  const soon = bottles.filter(b => {
    ...
    const s = drinkWindowStatus(b)
    return s?.status === 'wait' && !isNaN(from) && from <= soonYear
  })
