...
- 114 venues in Amanda's Places across 27 towns in 4 region groups (UK / Europe / Asia / Americas)
- 15 venue wine-list sources in `src/data/venueWineLists.js`
...
- `npm test` is currently clean (`124/124`).
...
The live `VENUES` array in `src/data/places.js` contains 114 venues across 27 towns in 4 region groups.
...
- Europe: Valencia, Málaga, Berlin, Munich, Arcachon, Cap Ferret
- Asia: Singapore
- Americas: New York, Miami, Panama City

Venue counts by town:
- Sheffield: 15 · Stannington: 3 · Walton-on-Thames: 2 · Stroud: 2 · Morpeth: 2
- Valencia: 29 · Berlin: 16 · London: 7 · New York: 2 · Chelmsford: 1
...
- release history and current production evidence are recorded in `STATUS.md`
...
3. A small number of Places venues still have no honest or reliably loadable official room image.
   Their text-led fallback treatment is deliberate and acceptable; do not weaken provenance to fill the gap.
